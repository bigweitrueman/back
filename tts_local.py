...
def generate_tts_wav_and_play(json_data, server_host='10.16.80.147', server_port=50000):
    ...
    # 应用3D丽音效果 (将单声道转为立体声并增强)
    audio_3d_enhanced = apply_3d_enhancement(audio_resampled_playback_float, PLAYBACK_RATE)
    
    # 应用最终标准化 (使用立体声版本)
    audio_np_playback = normalize_audio_stereo(audio_3d_enhanced)
    ...
    try:
        ...
        print(f"开始播放 (总帧数: {total_frames}, 时长: {total_frames/PLAYBACK_RATE:.2f}秒, 立体声3D丽音效果)")
    ...
    # 保存播放用的音频用于调试
    try:
        sf.write('playback_audio_3d.wav', audio_np_playback, PLAYBACK_RATE, 'PCM_16')
        print("已保存3D丽音播放音频用于调试: playback_audio_3d.wav")
    except Exception as e:
