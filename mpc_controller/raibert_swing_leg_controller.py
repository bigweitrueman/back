...
# 增大Kp以提供更强的速度误差修正,减少震荡
_KP = np.array([0.01, 0.01, 0.01]) * 9  # Kp=0.09,加强修正力度
...
class RaibertSwingLegController(leg_controller.LegController):
  ...
  def get_action(self) -> Mapping[Any, Any]:
    ...
    for leg_id, leg_state in enumerate(self._gait_generator.leg_state):
      ...
      # 关键修复: 改为加号,打破恶性循环
      # 当机器人后退时(hip_vel<0, target>0),应该让足端往前放来纠正
      # 原始公式用减号会加剧后退!
      ...
      # 调试: 打印FR腿的计算过程(降低频率)
      if leg_id == 0 and self._gait_generator.normalized_phase[leg_id] < 0.05:
          if not hasattr(self, '_last_debug_time'):
              self._last_debug_time = 0
          import time
          current_time = time.time()
          if current_time - self._last_debug_time >= 0.5:  # 每0.5秒打印一次
              self._last_debug_time = current_time
              print(f"\n[Raibert FR腿] phase={self._gait_generator.normalized_phase[leg_id]:.3f}")
              print(f"  desired_speed={self.desired_speed}")
              print(f"  hip_offset={hip_offset}")
              print(f"  hip_vel={hip_horizontal_velocity}")
              print(f"  target_vel={target_hip_horizontal_velocity}")
              print(f"  foot_target={foot_target_position}")
              relative_x = foot_target_position[0] - hip_offset[0]
              print(f"  相对髋关节X={relative_x:.4f} (负=向后, 正=向前) {'✓期望向前' if relative_x > 0 else '✗错误向后'}" )
