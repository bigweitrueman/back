...
_MPC_WEIGHTS = (5, 5, 0.2, 0, 0, 50, 0., 0., 1.0, 1.0, 1.0, 0., 0)  # 速度权重增加到1.0,平衡控制力度和稳定性
...
class TorqueStanceLegController(leg_controller.LegController):
  ...
  def get_action(self):
    ...
    # Use the robot/base COM position (world frame) when calling the MPC
    # solver. Passing `[0]` here was incorrect and produced zero/invalid
    # contact force outputs in our diagnostics.
    predicted_contact_forces = self._cpp_mpc.compute_contact_forces(
        np.asarray(self._robot.GetBasePosition(), dtype=np.float64),  # com_position
    ...
    for i in range(self._num_legs):
      force = np.array(
          predicted_contact_forces[i * _FORCE_DIMENSION:(i + 1) *
                                   _FORCE_DIMENSION])
      # 关键修复: MPC输出的力的方向定义与实际相反,需要反转X/Y分量
      force[0] = -force[0]  # 反转X方向力
      force[1] = -force[1]  # 反转Y方向力
      contact_forces[i] = force
    
    # 调试: 打印stance腿的X方向力(每0.5秒一次)
    if not hasattr(self, '_last_force_print_time'):
        self._last_force_print_time = 0
    import time
    current_time = time.time()
    if current_time - self._last_force_print_time >= 0.5:
        self._last_force_print_time = current_time
        stance_legs_x_forces = []
        for leg_id in range(self._num_legs):
            if foot_contact_state[leg_id]:
                fx = contact_forces[leg_id][0]
                stance_legs_x_forces.append((leg_id, fx))
        if stance_legs_x_forces:
            print(f"[MPC Stance Forces X] desired_vel={desired_com_velocity[0]:.3f}, forces(反转后)={stance_legs_x_forces}")
    ...
    for leg_id, force in contact_forces.items():
      ...
      # CRITICAL FIX: Add PD control to stance legs for stability
      # Use DESIRED STANDING ANGLES as target, not current angles!
      motors_per_leg = self._robot.num_motors // self._robot.num_legs
      
      # Standing pose angles (from URDF defaults): [hip_x, hip_y, knee]
      standing_angles = np.array([0.0, -1.178, 1.658])
      
      # PD gains for stance legs (provide stability while allowing MPC torque)
      kp_stance = np.array([100.0, 100.0, 150.0])
      kd_stance = np.array([1.0, 2.0, 2.0])
      
      for i, joint_id in enumerate(range(leg_id * motors_per_leg, (leg_id + 1) * motors_per_leg)):
        # Combine PD control (targeting standing pose) with feedforward torque from MPC
        action[joint_id] = (
            standing_angles[i],       # target position (standing pose)
            kp_stance[i],             # Kp
            0.0,                      # target velocity
            kd_stance[i],             # Kd
            motor_torques.get(joint_id, 0.0)  # feedforward torque from MPC
        )
